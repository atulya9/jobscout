...
4. Open this folder as the workspace in OpenCode, Gemini CLI/Antigravity, Grok Build, or Claude Code. Review and trust the workspace MCP configuration when prompted.
...
6. Sign in manually to portals that need an account—typically LinkedIn, Naukri, Cutshort, Instahyre, Wellfound, Arc.dev, and sometimes Indeed/Hirist. Never place passwords or cookies in this workspace.
...
The first MCP launch through `npx` downloads the server packages. Restart the harness after changing its MCP config. Grok Build users should trust the project when prompted, can verify discovery with `grok inspect`, and can diagnose servers with `grok mcp doctor chrome-devtools` or `grok mcp doctor playwright`.
